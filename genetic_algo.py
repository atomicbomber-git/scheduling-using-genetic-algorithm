...
class GeneticAlgoV2:
    ...
    def crossover_population(self, pop: Population):
        ...
        for _ in range(self.elite_schedules_count, self.population_size):
            ...
            if random.random() < self.crossover_rate:
                child_schedule = self.get_schedule_by_crossover(
                    parent_schedule_1,
                    parent_schedule_2
                )

                candidate_schedule = self.mutate_schedule(child_schedule)
            else:
                candidate_schedule = max([parent_schedule_1, parent_schedule_2], key=lambda schedule: schedule.fitness)

            crossover_pop.schedules.append(candidate_schedule)

        return crossover_pop

    def get_schedule_by_crossover(self, parent_schedule_1: Schedule, parent_schedule_2: Schedule):
        crossed_acs = one_point_crossover(parent_schedule_1.academic_classes, parent_schedule_2.academic_classes)
        new_schedule = Schedule(0, data=None, academic_classes=[], fitness=0)
        new_schedule.academic_classes = [ac.clone() for ac in crossed_acs]
        new_schedule.calculate_and_cache_fitness()
        return new_schedule
